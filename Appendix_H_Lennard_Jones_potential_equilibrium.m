% Title: Random Motion of Multiple Particles with Lennard-Jones Equilibrium
% Author: Gopi Reddy Rajendran
% Description:
% This MATLAB code simulates the motion of multiple particles in a 1D space with Lennard-Jones equilibrium forces.
% The particles are initially distributed within a 1D box of size 'L'.
% Each particle undergoes a random walk with Lennard-Jones equilibrium interactions.
...
for i = 1:N
    xcurr = [3 4.1224]; % Initial positions (customize as needed)
...
if ~exist('LJ_Equilibrium', 'dir')
    mkdir('LJ_Equilibrium')
end

% Create VideoWriter object for saving animation
outputVideo = VideoWriter('LJ_Equilibrium.avi');
...
title({'Lennard-Jones Equilibrium Simulation'; 'step = 0'});
axis([0 L -L L]);
drawnow;
frame = getframe(gcf);
writeVideo(outputVideo, frame);
saveas(fig, fullfile('LJ_Equilibrium', sprintf('Initial.png')))
close(gcf);
...
for n = 1:nSteps
    ...
    title({'Lennard-Jones Equilibrium Simulation'; ['step = ', num2str(n)]});
    axis([0 L -L L]);
    drawnow;
    frame = getframe(gcf);
    writeVideo(outputVideo, frame);
    ...
    if mod(n, 10) == 0
        saveas(fig, fullfile('LJ_Equilibrium', sprintf('step%d.png', n)))
    end
