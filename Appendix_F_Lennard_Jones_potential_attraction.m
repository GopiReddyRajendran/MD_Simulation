% Title: Random Motion of Multiple Particles with Lennard-Jones Attraction
% Author: Gopi Reddy Rajendran
% Description:
% This MATLAB code simulates the motion of multiple particles in a 1D space with Lennard-Jones attraction forces.
% The particles are initially distributed within a 1D box of size 'L'.
% Each particle undergoes a random walk with Lennard-Jones attraction interactions.
...
for i = 1:N
    xcurr = [3 5.5]; % Initial positions (customize as needed)
...
if ~exist('LJ_Attraction', 'dir')
    mkdir('LJ_Attraction')
end

% Create VideoWriter object for saving animation
outputVideo = VideoWriter('LJ_Attraction.avi');
...
title({'Lennard-Jones Attraction Simulation'; 'step = 0'});
axis([0 L -L L]);
drawnow;
frame = getframe(gcf);
writeVideo(outputVideo, frame);
saveas(fig, fullfile('LJ_Attraction', sprintf('Initial.png')))
close(gcf);
...
for n = 1:nSteps
    ...
    title({'Lennard-Jones Attraction Simulation'; ['step = ', num2str(n)]});
    axis([0 L -L L]);
    drawnow;
    frame = getframe(gcf);
    writeVideo(outputVideo, frame);
    ...
    if mod(n, 10) == 0
        saveas(fig, fullfile('LJ_Attraction', sprintf('step%d.png', n)))
    end
