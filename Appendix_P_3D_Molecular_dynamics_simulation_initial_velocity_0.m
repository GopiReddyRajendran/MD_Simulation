...
% simulation with initial velocity as 0 of particles interacting with a Lennard-Jones potential.
...
outputVideo = VideoWriter('3D0.avi');
...
saveas(fig, fullfile('3D_0', sprintf('Initial.png')))
close(gcf);
...
for n = 1:nSteps
    ...
    writeVideo(outputVideo, frame);
    if mod(n,10) == 0
        saveas(fig, fullfile('3D_0', sprintf('step%d.png', n)))
    end
close(gcf);    
end
% Finish video
